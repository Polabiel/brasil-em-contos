    ...
    <IconButton
      ...
      sx={{
        ...
        '&:hover': {
          ...
          borderColor: isBookmarked ? 'var(--cv-brazilYellow)' : 'var(--cv-brazilYellow)',
          color: isBookmarked ? 'var(--cv-textPrimary)' : 'var(--cv-brazilYellow)',
        }
      }}
    >
      <i className={`fas ${isBookmarked ? 'fa-bookmark' : 'fa-bookmark'}`} style={{ fontSize: '1.2rem' }} />
    </IconButton>
  );
}
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      {showShareMenu && (
        ...
        >
          <Stack spacing={1}>
            <Typography level="body-sm" sx={{ color: 'var(--cv-textMuted70)', fontWeight: 600, mb: 1 }}>
              Compartilhar
            </Typography>
            
            {[
              { platform: 'whatsapp', icon: 'fab fa-whatsapp', label: 'WhatsApp', color: '#25D366' },
              { platform: 'facebook', icon: 'fab fa-facebook-f', label: 'Facebook', color: '#1877F2' },
              { platform: 'twitter', icon: 'fab fa-twitter', label: 'Twitter', color: '#1DA1F2' },
              { platform: 'linkedin', icon: 'fab fa-linkedin-in', label: 'LinkedIn', color: '#0A66C2' },
            ].map((item) => (
              <Box
                key={item.platform}
                component="button"
