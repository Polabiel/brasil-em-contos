...
export default function AuthorPage() {
 ...
 const { data: author, isLoading, error } = api.author.bySlug.useQuery({ slug: slug ?? '' }, { enabled: !!slug });
    ...
    <CardContent>
     {isLoading ? (
      ...
      <Stack spacing={2}>
       ...
       <Typography level="h4" sx={{ fontWeight: 700 }}>{author.name}</Typography>
       {author.period && <Typography level="body-sm" sx={{ color: 'var(--cv-textMuted80)' }}>{author.period}</Typography>}
       {author.bio && <Typography level="body-md">{author.bio}</Typography>}

       {author.books && author.books.length > 0 && (
        <Box>
         <Typography level="h4" sx={{ mt: 2, mb: 1 }}>Livros</Typography>
         <Stack spacing={1}>
          {author.books.map((b) => (
           <Typography key={b.id} level="body-sm">{b.title} {b.year ? `(${b.year})` : ''}</Typography>
          ))}
         </Stack>
        </Box>
       )}
