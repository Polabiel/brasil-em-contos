...
export const authorRouter = createTRPCRouter({
  list: publicProcedure.query(async ({ ctx }) => {
    ...
    const sql = `
      SELECT a.id, a.name, a.period, a.slug, a.image, a.imagemime,
    ...
    interface AuthorRow { id: number; name: string; period: string | null; books: string | BookRow[]; slug?: string | null; image?: string | null; imagemine?: string | null }
    ...
    return (rawRows as unknown[]).map((r) => {
      ...
      return {
        ...
        imageMime: row.imagemine ?? undefined,
  ...
  bySlug: publicProcedure.input(z.object({ slug: z.string().min(1) })).query(async ({ ctx, input }) => {
    const row = await ctx.db.$queryRawUnsafe(
      `SELECT a.id, a.name, a.period, a.bio, a.image, a.imagemime, COALESCE(json_agg(json_build_object('id', b.id, 'title', b.title, 'year', b.year)) FILTER (WHERE b.id IS NOT NULL), '[]') as books FROM "Author" a LEFT JOIN "Book" b ON b."authorId" = a.id WHERE a.slug = $1 GROUP BY a.id`,
      input.slug,
    );
    const maybeRaw = (row as unknown[])[0] as Record<string, unknown> | undefined;
    if (!maybeRaw) return null;
    ...
    const period = (typeof maybeRaw.period === 'string' || typeof maybeRaw.period === 'number') ? String(maybeRaw.period) : undefined;
    const bio = (typeof maybeRaw.bio === 'string' || typeof maybeRaw.bio === 'number') ? String(maybeRaw.bio) : undefined;
    const image = (typeof maybeRaw.image === 'string' || typeof maybeRaw.image === 'number') ? String(maybeRaw.image) : undefined;
    const imageMime = (typeof maybeRaw.imagemime === 'string' || typeof maybeRaw.imagemime === 'number') ? String(maybeRaw.imagemime) : undefined;
    let booksParsed: Array<{ id: number; title: string; year?: number }> = [];
    try {
      const rawBooks = maybeRaw.books;
      const booksCandidate: unknown = typeof rawBooks === 'string' ? JSON.parse(String(rawBooks)) : (Array.isArray(rawBooks) ? rawBooks : []);
    ...
    return { id: Number(id), name: String(name), period, bio, image, imageMime, books: booksParsed };
  }),
